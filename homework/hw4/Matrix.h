...
public:
    ...
    void operator+=(Matrix<rows, cols> const &r) {
    ...
    // Just for comparing the 2x2 specialization
    double determinant_no_specialization() const {
		double val = 0;
		for (int i = 0; i < rows; i++) {
			val += (i % 2 ? -1 : 1)
				* data[i][0]
...
// Specialization for 2 by 2 determinant
template<>
double
